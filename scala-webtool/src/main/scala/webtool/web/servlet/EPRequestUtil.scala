...
import webtool.util.nio.ChannelUtil
import javax.servlet.http.HttpServletRequest
import webtool.Configurator
import webtool.Key
import webtool.DefaultVal

object EPRequestUtil extends LazyLogging {
  ...
  def getQueryString(request: HttpServletRequest): String = {
    val encoding = getCharacterEncoding(request).orElse(Configurator.Charset).getOrElse(DefaultVal.Charset)
