...
object GenFromJSON13Test {
  ...
  def test1() {
    ...
    val ep = GenFromJSON13.gen(json13Str, Some("UTF-8"))
    val parameters = ep.parameters
    val dataset = ep.getDataset("1")

    println(dataset.map { ds => ds.setMkString(JSON13MkString) })
